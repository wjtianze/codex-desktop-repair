...
| Chat 流式摘要与快速聊天操作 | 11 项完整摘要与搜索组件生命周期、8 项操作状态保护、8 项完整消息视图与原生编辑验证通过 |
| 历史计时与进度补取 | 13 项辅助模块、8 项原生视图与读取入口验证通过 |
| 保存窗口与分块写入 | 16 项保存事务、8 项原生下载入口和控件验证通过 |
| 互动演示兼容 | 17 项独立检查、1 项原生接入检查和 2 项真实浏览器布局与交互检查通过 |
| 演示逐步生成与原生容器 | 4 项解析与会话、4 项组件、1 项原生流、1 项浏览器和 2 项原生容器检查通过 |
| 搜索来源辅助模块 | 4 项检查通过，覆盖去重、来源边界和原生链接动作 |
| 合计 | 326 项通过；包含 134 项可独立运行的跨平台测试，192 项使用官方资源的行为测试 |
| 真实应用资源 | 完整修复版 8807 项、无侧栏版 8806 项，逐项校验通过 |
...
1.0.5 的本机候选版通过 28 项安装事务校验，42 项既有模型偏好全部保留；实际运行中确认新版 genui 与搜索模块已加载。用户随后确认交互式演示的问题已修复。两种语言分别通过 326 项检查；搜索过程以原生组件及合成数据验证，来源仅随服务端下发显示。详见 [演示逐步生成](PROGRESSIVE_VISUALIZATION.md) 和 [搜索过程显示](SEARCH_PROGRESS.md)。
