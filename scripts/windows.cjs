...
function powershell(code,extraEnv={}){
 ...
 const result=spawnSync(path.join(process.env.SystemRoot,'System32','WindowsPowerShell','v1.0','powershell.exe'),['-NoLogo','-NoProfile','-NonInteractive','-Command',command],{windowsHide:true,encoding:'utf8',env:{...process.env,...extraEnv},maxBuffer:4*1024*1024});
...
function runningClients(){const text=powershell("ConvertTo-Json -Compress -InputObject @(Get-CimInstance Win32_Process -Filter \"Name='ChatGPT.exe'\" | Select-Object ProcessId,ExecutablePath,CommandLine)");return text?JSON.parse(text):[]}
...
function shortcut(file,target,icon){powershell("$s=New-Object -ComObject WScript.Shell;$l=$s.CreateShortcut($env:DESKTOP_REPAIR_SHORTCUT);$l.TargetPath=$env:DESKTOP_REPAIR_TARGET;$l.WorkingDirectory=Split-Path -Parent $env:DESKTOP_REPAIR_TARGET;$l.IconLocation=$env:DESKTOP_REPAIR_ICON+',0';$l.WindowStyle=1;$l.Description='ChatGPT';$l.Save();Add-Type -TypeDefinition $env:DESKTOP_REPAIR_SHORTCUT_TYPE;[RepairShortcutIdentity]::Set($env:DESKTOP_REPAIR_SHORTCUT,$env:DESKTOP_REPAIR_APP_ID)",{DESKTOP_REPAIR_SHORTCUT:file,DESKTOP_REPAIR_TARGET:target,DESKTOP_REPAIR_ICON:icon,DESKTOP_REPAIR_SHORTCUT_TYPE:require('./shortcut-identity.cjs').source,DESKTOP_REPAIR_APP_ID:require('./shortcut-identity.cjs').APP_ID})}
