...
function main(){
 ...
 if(action==='uninstall'){
  ...
  locked(base,()=>{let count=0;const seen=new Set();while(fs.existsSync(stateFile)){const state=read(stateFile);assert.ok(count<128&&!seen.has(state.transaction),'恢复记录存在循环或超过单次恢复上限');seen.add(state.transaction);const result=transaction.rollback(state.transaction,{allowExternalChanges:true});count++;console.log('已恢复一份安装记录；保留备份：'+result.backupDirectory);if(result.skippedExternal)console.log('部分浏览器组件在安装后已变更，已保留当前文件。')}console.log('恢复完成。聊天和账号数据未清除，备份仍保留在本地。')});return;
 }
