...
const native=source.match(/if\(m\.type===`web-search`\)[\s\S]*?jsx\)\((\w+),\{item:m\}/)?.[1];
const patched=source.match(/__localWebSearchProgress\(\{jsx:Q\.jsx,NativeSearch:(\w+),item:o/ )?.[1];
...
vm.runInNewContext(expression,{Q:{jsx(){}},[native]:Search,Kgt(){throw Error('Memory component must not render search data')},o:item,v:false,Ze(){},__localOpenSearchLink(){},__localWebSearchProgress:options=>{selected=options.NativeSearch;assert.equal(options.item,item)}});
...
const nativeSources='$br';
assert.ok(nativeFunction(initial,'__local_uncached_searchGroups').includes('search_result_groups'));
const collector=nativeFunction(initial,'__local_uncached_RKr');
assert.ok(collector.includes(nativeSources+'(e.metadata,gO(e))'),'Search metadata must use the canonical source parser');
const object=x=>x&&typeof x==='object'&&!Array.isArray(x)?x:null;
const context={URL,__localMemoObject:memoizeObjectResult,$Xr(){},gO:e=>e.id,bO:object,xO:x=>typeof x==='string'&&x.trim()?x:null,yO:x=>Array.isArray(x)?x:[],NCr:()=>[],gar:e=>e.output??[],oxr:{safeParse:x=>({success:!!object(x),data:x})},sxr:{safeParse:x=>({success:Array.isArray(x?.entries),data:x})},mbr:()=>null,wbr:()=>false,Cbr:()=>false,Tbr:()=>false,Ebr:()=>false,PO:e=>e.title,ryr:e=>{try{return /^https?:$/.test(new URL(e.url).protocol)?e:null}catch{return null}},cxr:{safeParse:x=>({success:Number.isFinite(x),data:new Date(x*1000)})}};
const helperNames=['FCr','ICr','$br','__local_uncached_searchGroups','hbr','fA','axr'];
const definitions=helperNames.map(n=>nativeFunction(initial,n)).join('\n');
const collect=vm.runInNewContext(definitions+collector+nativeFunction(initial,"CCr")+';CCr',context);
