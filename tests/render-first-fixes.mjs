...
test('API context stuffing page images remain retrieval context instead of an empty generated-image gallery',()=>{const m=message('api_tool','tool',50);m.metadata.command='context_stuff';assert.equal(before(m,{isActiveTurn:false,isStreaming:false}).length,50);assert.equal(run(m).length,0)});
test('API tools without the exact internal-context command and assistant images remain visible',()=>{for(const command of[undefined,'image_generation','context_stuff_other']){const m=message('api_tool');m.metadata.command=command;assert.deepEqual(normalized(run(m)),normalized(before(m,{isActiveTurn:false,isStreaming:false})))}const m=message('api_tool','assistant');m.metadata.command='context_stuff';assert.equal(run(m).length,1)});
...
test('Native image-generation calls and their pending status remain intact',()=>{const m={...message(null,'assistant',0),output:[{type:'image_generation_call',id:'g',status:'in_progress'}]};assert.deepEqual(normalized(run(m)),normalized(before(m,{isActiveTurn:false,isStreaming:false})));assert.equal(after(m,{isActiveTurn:true,isStreaming:true})[0].status,'in_progress');assert.equal(run(m).length,0)});
