...
import {createRequire} from 'node:module';import {boundedTurnRange,retainTurnRange} from '../assets/local-chat-block-visibility-v1.mjs';
...
const file=path.join(require('../scripts/windows.cjs').packageInfo().InstallLocation,'app/resources/app.asar'),fd=fs.openSync(file,'r');let source;try{const h=p.archiveHeader(fd),e=p.getEntry(h.tree,'webview/assets/thread-virtualizer-3203ea3d2081.js');source=p.readExact(fd,e.size,h.offset+Number(e.offset)).toString()}finally{fs.closeSync(fd)}
...
assert.equal(retainTurnRange({startIndex:0,endIndex:3},{startIndex:2,endIndex:3},large,800),false);
assert.equal(retainTurnRange({startIndex:0,endIndex:3},{startIndex:1,endIndex:3},layout([20,20,20]),800),true);
assert.equal(retainTurnRange({startIndex:0,endIndex:4},{startIndex:0,endIndex:3},large,800),false);
...
const s=fs.readFileSync('build/fixtures/render/patches/virtual-list.js','utf8');assert.equal((s.match(/__localBoundedTurnRange\(R,/g)??[]).length,3);assert.ok(s.includes('R({distanceFromBottomPx:e,layout:t,overscanCount:0,viewportHeightPx:i})'));
