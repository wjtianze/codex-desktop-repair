...
function fixture(patched=true){
 ...
 const cache=new Map();const context={rg:{c:n=>{if(!cache.has(n))cache.set(n,memo(n));return cache.get(n)}},Q:{jsx,jsxs:jsx,Fragment:'Fragment'},G:()=>({data:undefined}),li:"li",zl:()=>null,__localWebSearchProgress:renderWebSearchProgress,qe:()=>{},Wp:"NativeSearch",jc:'NativeSearch',la:()=>{}};
 for(const name of["Qc","Xh","ie","_t","qn","fc","Bd","Ih"])context[name]=name;
 const render=vm.runInNewContext(section(source,'function Jh(','function Yh(')+';Jh',context);
 const hidden=vm.runInNewContext(section(source,'function Gh(','function Jh(')+';Gh',{...context,Jh:render});
 const activity=fs.readFileSync(path.join(root,'raw','activity.js'),'utf8');
 const native=section(activity,'function B_(','function U_(')+nativeFunction(activity,'V_')+nativeFunction(activity,'H_');
 let key,instance;
 function disclose(node){
  if(node.type!=="Qc")return{node,body:null};
  if(key!==node.key){key=node.key;instance={cache:memo(37),initialized:false};
   const ctx={G_:{c:()=>instance.cache},K_:{useState:init=>{if(!instance.initialized){instance.state=typeof init==='function'?init():init;instance.initialized=true}return[instance.state,value=>{instance.state=typeof value==='function'?value(instance.state):value}]}},q_:{jsx},ge:{div:'Motion'},_a:{},requestAnimationFrame:fn=>fn()};
   for(const name of["U_","qs","_o","Ps"])ctx[name]=name;
   instance.render=vm.runInNewContext(native+';B_',ctx);
  }
...
test('No summary data keeps the Thinking indicator without an empty expanded body',()=>{const f=fixture();for(let i=0;i<3;i++){const r=f.render([thought()]);assert.equal(r.node.key,'active');assert.equal(r.node.props.summary.type,"ie");assert.equal(r.node.props.canExpand,false);assert.equal(r.body,null)}});
...
test('Work conversations retain their separate native activity route',()=>{const r=fixture().render([thought('Summary')],{isWorkConversation:true});assert.equal(r.node.type,"Xh");assert.equal(r.node.props.items[0].content,'Summary')});
