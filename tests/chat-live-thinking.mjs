...
test('No summary data keeps the Thinking indicator without an empty expanded body',()=>{const f=fixture();for(let i=0;i<3;i++){const r=f.render([thought()]);assert.equal(r.node.key,'active-empty');assert.equal(r.node.props.summary.type,"Qo");assert.equal(r.node.props.canExpand,false);assert.equal(r.body,null)}});
...
test('Work conversations retain their separate native activity route',()=>{const r=fixture().render([thought('Summary')],{isWorkConversation:true});assert.equal(r.node.type,"hx");assert.equal(r.node.props.items[0].content,'Summary')});
