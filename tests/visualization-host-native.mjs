...
const source=fs.readFileSync(new URL('../build/fixtures/render/patches/initial.js',import.meta.url),'utf8'),jsx=(type,props)=>({type,props});
const section=(a,b)=>nativeFunction(source,/function\s*([\w$]+)\(/.exec(a)[1]);
function native(context={}){return vm.runInNewContext(nativeFunction(source,"Two")+';Two',{Ewo:{c:n=>Array(n).fill(Symbol.for('react.memo_cache_sentinel'))},lH:()=>({conversationId:'fixture',isStreaming:true}),ZCo:{safeParse:r=>({success:r?.type==='client_defined_widget'&&r?.category==='app_block'&&typeof r?.data?.content==='string',data:r})},Dwo:{jsx},bwo:'NativeVisualization',...context})}
...
assert.equal(native()({reference:{data:{content:'invalid envelope'}}}),null);assert.equal(native({lH:()=>null})({reference:reference({content:'<div/>'})}),null);
console.log('PASS Native validation and missing-context gates remain effective');
class IFrame{setAttribute(key,value){this[key]=value}};const create=vm.runInNewContext(section('function nmo(','var S7e=')+';nmo',{document:{createElement:name=>name==='iframe'?new IFrame():{style:{},setAttribute(key,value){this[key]=value}}},HTMLIFrameElement:IFrame,voe:id=>"fixture:"+id,LSe:id=>'fixture:'+id});
