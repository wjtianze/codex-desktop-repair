...
function native(context={}){return vm.runInNewContext(nativeFunction(source,"XP")+';XP',{Ovt:{c:n=>Array(n).fill(Symbol.for('react.memo_cache_sentinel'))},Xb:()=>({conversationId:'fixture',isStreaming:true}),nvt:{safeParse:r=>({success:r?.type==='client_defined_widget'&&r?.category==='app_block'&&typeof r?.data?.content==='string',data:r})},kvt:{jsx},wvt:'NativeVisualization',...context})}
...
assert.equal(native()({reference:{data:{content:'invalid envelope'}}}),null);assert.equal(native({Xb:()=>null})({reference:reference({content:'<div/>'})}),null);
console.log('PASS Native validation and missing-context gates remain effective');
class IFrame{setAttribute(key,value){this[key]=value}};const create=vm.runInNewContext(section('function rnt(','var S7e=')+';rnt',{document:{createElement:name=>name==='iframe'?new IFrame():{style:{},setAttribute(key,value){this[key]=value}}},HTMLIFrameElement:IFrame,Yre:id=>"fixture:"+id,Lie:id=>'fixture:'+id});
