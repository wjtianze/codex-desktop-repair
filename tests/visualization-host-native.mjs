...
const section=(a,b)=>{const at=source.indexOf(a),end=source.indexOf(b,at);assert.ok(at>=0&&end>at);return source.slice(at,end)};
function native(context={}){return vm.runInNewContext(section('function TXn(','function EXn(')+';TXn',{DXn:{c:n=>Array(n).fill(Symbol.for('react.memo_cache_sentinel'))},zx:()=>({}),$_:{},uc:()=>({conversationId:'fixture',messageId:'message'}),o0:{useContext:()=>null},Cr:{},s0:{jsx,Fragment:'Fragment'},rRt:'NativeVisualization',__localVisualizationWantsWide:visualizationWantsWide,...context})}
...
assert.equal(native({uc:()=>({shouldBlockExternalEgress:true})})({reference:{data:{content:'<div class="viz-grid"></div>'}}}),null);
console.log('PASS Native visualization requests wide layout for multiple columns and keeps existing content restrictions');
class IFrame{setAttribute(key,value){this[key]=value}};const create=vm.runInNewContext(section('function vLt(','var yLt=')+';vLt',{document:{createElement:name=>name==='iframe'?new IFrame():{style:{},setAttribute(key,value){this[key]=value}}},HTMLIFrameElement:IFrame,Bde:id=>'fixture:'+id});
