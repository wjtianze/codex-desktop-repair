...
function native(kind){const s=fs.readFileSync('build/fixtures/render/'+kind+'/state-store.js','utf8'),writes=[],warnings=[];const context={__localDraftPersistence:require('../assets/local-draft-persistence-v1.cjs'),rD:()=>false,nD:new Set(),$E:{},f:fs,p:{...fs.promises,writeFile:async(file,text,...rest)=>{writes.push(Buffer.byteLength(text));return fs.promises.writeFile(file,text,...rest)}},u:path,d:crypto,h:{setTimeout:async()=>{}},WE:[],GE:()=>({warning:(...args)=>warnings.push(args)})};const a=s.indexOf('function iD('),b=s.indexOf('function fD(',a);return {...vm.runInNewContext(s.slice(a,b)+';({read:iD,write:sD,strict:cD})',context),writes,warnings}}
(async()=>{
