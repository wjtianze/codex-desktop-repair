...
function native(kind){const s=fs.readFileSync('build/fixtures/render/'+kind+'/state-store.js','utf8'),writes=[],warnings=[];const context={__localDraftPersistence:require('../assets/local-draft-persistence-v1.cjs'),lE:()=>false,cE:new Set(),aE:{},f:fs,p:{...fs.promises,writeFile:async(file,text,...rest)=>{writes.push(Buffer.byteLength(text));return fs.promises.writeFile(file,text,...rest)}},l:path,d:crypto,ZT:()=>({warning:(...args)=>warnings.push(args)})};const a=s.indexOf('function uE('),b=s.indexOf('function yE(',a);return {...vm.runInNewContext(s.slice(a,b)+';({read:uE,write:hE,strict:mE})',context),writes,warnings}}
(async()=>{
