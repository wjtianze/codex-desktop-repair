...
for(const kind of ['raw','patches']){
 const render=load(kind,'rn',{cn:compiler,$:{jsx},an:'native-chat-code',He:'lazy-placeholder',Pt:()=>null});
...
const context={cn:compiler,ln:state,$:{jsx,jsxs:jsx,Fragment:'fragment'},de:()=>({get(){},set(){}}),te:'scope',r:()=>format,we:()=>null,We:'server-id',me:()=>false,Ke:'wrap',n:()=>({get:()=>false}),se:()=>false,Pt:()=>null,Ee:l=>({value:l,label:l==='cpp'?'C++':l}),N:'snippet',ne:'spinner',tn:{InlineCodePane:'inline'},y:{CodeBlock:'block'},Ce:(...v)=>v.filter(Boolean).join(' ')};
