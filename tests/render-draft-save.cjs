...
const root=path.resolve(__dirname,'../build/fixtures/render'),source=fs.readFileSync(path.join(root,'patches/initial.js'),'utf8'),a=source.indexOf('function Lai('),b=source.indexOf('var Rai,',a),calls=[];
const create=vm.runInNewContext(source.slice(a,b)+';Lai',{ME:()=>({}),Y:()=>({}),Q:{},Dv:()=>({}),Av:(key,value,delay)=>calls.push({key,value,delay}),Rai:250});
