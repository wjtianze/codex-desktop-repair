...
const root=path.resolve(__dirname,'../build/fixtures/render'),source=fs.readFileSync(path.join(root,'patches/initial.js'),'utf8'),a=source.indexOf('function KNr('),b=source.indexOf('var qNr,',a),calls=[];
const create=vm.runInNewContext(nativeFunction(source,'KNr')+';KNr',{xv:()=>({}),sf:()=>({}),$:{},Fp:()=>({}),Lp:(key,value,delay)=>calls.push({key,value,delay}),qNr:250});
