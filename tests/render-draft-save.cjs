...
const root=path.resolve(__dirname,'../build/fixtures/render'),source=fs.readFileSync(path.join(root,'patches/initial.js'),'utf8'),a=source.indexOf('function ZNr('),b=source.indexOf('var QNr,',a),calls=[];
const create=vm.runInNewContext(source.slice(a,b)+';ZNr',{fw:()=>({}),X:()=>({}),Q:{},jh:()=>({}),Nh:(key,value,delay)=>calls.push({key,value,delay}),QNr:250});
