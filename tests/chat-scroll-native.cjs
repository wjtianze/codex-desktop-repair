...
const start=source.indexOf('var __localChatTurns;'),end=source.indexOf('function qc(',start);
const calls=[],Native=()=>{},Wrapped=()=>{},controller=()=>{};
const render=vm.runInNewContext(source.slice(start,end)+';Kc',{
 __localCreateChatTurns:options=>{calls.push(options);return Wrapped},__localChatPromptRail:'rail',Q:{useRef:value=>({current:value}),useCallback:fn=>fn},$:{jsx:(type,props,key)=>({type,props,key}),jsxs:(type,props,key)=>({type,props,key}),Fragment:'fragment'},eo:Native,__localUseChatScrollController:controller,M:()=>[{messageId:'retained'}],Z:{c:n=>Array(n).fill(Symbol.for('react.memo_cache_sentinel'))},il:'retained-key',qc:()=>{}
});
