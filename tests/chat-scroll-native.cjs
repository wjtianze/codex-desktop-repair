...
const start=source.indexOf('var __localChatTurns;'),end=source.indexOf('function ou(',start);
const calls=[],Native=()=>{},Wrapped=()=>{},controller=()=>{};
const render=vm.runInNewContext(source.slice(start,end)+';au',{
 __localCreateChatTurns:options=>{calls.push(options);return Wrapped},__localChatPromptRail:'rail',Q:{useRef:value=>({current:value}),useCallback:Xe=>Xe},$:{jsx:(type,props,key)=>({type,props,key}),jsxs:(type,props,key)=>({type,props,key}),Fragment:'fragment'},Ro:Native,__localUseChatScrollController:controller,m:()=>[{messageId:'retained'}],Z:{c:n=>Array(n).fill(Symbol.for('react.memo_cache_sentinel'))},vu:'retained-key',ou:()=>{}
});
...
const page=source.slice(source.indexOf('function ru('),source.indexOf('function iu('));
...
assert.ok(page.includes('t[108]!=='+key+'?'),'Memoized content must invalidate when the scroll conversation ID changes');
assert.ok(page.includes('t[108]='+key+','),'Memoized content must retain the same conversation ID');
