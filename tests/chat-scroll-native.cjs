...
const start=source.indexOf('var __localChatTurns;'),end=source.indexOf('function Od(',start);
const calls=[],Native=()=>{},Wrapped=()=>{},controller=()=>{};
const render=vm.runInNewContext(source.slice(start,end)+';Dd',{
 __localCreateChatTurns:options=>{calls.push(options);return Wrapped},__localChatPromptRail:'rail',Q:{useRef:value=>({current:value}),useCallback:Xe=>Xe},$:{jsx:(type,props,key)=>({type,props,key}),jsxs:(type,props,key)=>({type,props,key}),Fragment:'fragment'},cc:Native,__localUseChatScrollController:controller,U:()=>[{messageId:'retained'}],Z:{c:n=>Array(n).fill(Symbol.for('react.memo_cache_sentinel'))},Bd:'retained-key',Od:()=>{}
});
...
const page=source.slice(source.indexOf('function Td('),source.indexOf('function Ed('));
...
assert.ok(page.includes('t[114]!=='+key+'?'),'Memoized content must invalidate when the scroll conversation ID changes');
assert.ok(page.includes('t[114]='+key+','),'Memoized content must retain the same conversation ID');
