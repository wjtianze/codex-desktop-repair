...
function prepare(source){
 ...
 for(const kind of ['original','patched'])for(const id of ['initial','primary','conversation','viewer','logger','state-store','quick-chat','quick-transcript'])write('render/'+(kind==='original'?'raw':'patches')+'/'+id+'.js',read(id+'.'+kind+'.js'));
